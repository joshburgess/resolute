...
| **stalwart** | Compile-time checked queries. 7 query macros with type overrides (`"col: Type"`), `Executor` trait, `atomic()` with savepoint nesting, named params, `FromRow` (with `skip`/`default`/`json`/`try_from`/`flatten`), `PgEnum` (string + integer-backed), `PgComposite`, `PgDomain` (with array OID inheritance), `TypedPool` with lifecycle hooks, `PgListener`, streaming, pipelining, COPY, retry, auto-reconnect, metrics. |
...
**Rich FromRow derive.** Beyond basic column mapping, stalwart's `FromRow` supports `skip` (use defaults), `default` (fall back on missing/NULL), `json` (serde deserialization), `try_from` (type conversion with validation), and `flatten` (nested struct composition). sqlx's `FromRow` has similar attributes but stalwart's `default` handles both missing columns and NULL values.
