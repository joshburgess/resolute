...
function printTable(title: string, rows: Row[]): void {
  ...
  for (const r of rows) {
    console.log(
      [r.label, r.ops, r.meanUs, r.p50Us, r.p99Us].map((v, i) => pad(v, widths[i]!)).join("  "),
    )
  }
}
...
r2.push(
  await run("pg-wired     ", async () => {
    await wired.query("SELECT id, name, score FROM bench_rows WHERE id = $1", [1n], [OID.INT8])
  }),
)
...
r4.push(
  await run("pg-wired pipe", async () => {
    ...
    for (let i = 0; i < BATCH; i++) {
      pipe.push("SELECT id FROM bench_rows WHERE id = $1", [BigInt(i + 1)], [OID.INT8])
    }
    await pipe.execute()
  }),
)
