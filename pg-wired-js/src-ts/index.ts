...
wrapMethods(
  native.Connection.prototype as object,
...
export async function connect(
  optsOrUrl: ConnectOptions | string,
): Promise<Connection> {
  try {
...
function makeTxHandle(
  ...
  handle.savepoint = async function (
    ...
    const rawName =
      typeof nameOrFn === 'string' ? nameOrFn : `sp_${++state.spCounter}`;
...
(native.Connection.prototype as unknown as Record<string, unknown>).transaction =
  function (
    this: Connection,
...
const _nativePipelinePush =
  (native.Pipeline.prototype as unknown as { push: NativePipelinePush }).push;
...
export const Notifications =
  native.Notifications as unknown as new () => Notifications;
