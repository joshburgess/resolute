//! CLI tool for stalwart: offline cache management and database migrations.
//!
//! Usage:
//!   stalwart-cli prepare               # Cache query metadata for offline builds
//!   stalwart-cli check                 # Verify cached queries against DB
//!   stalwart-cli migrate create <name> # Create a new migration
//!   stalwart-cli migrate run           # Run pending migrations
//!   stalwart-cli migrate revert        # Revert the last migration
//!   stalwart-cli migrate status        # Show migration status
...
#[derive(Parser)]
#[command(name = "stalwart-cli", about = "Offline cache management for stalwart query!() macro")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
async fn check(database_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !cache_dir.is_dir() {
        println!("No .sqlx cache directory found. Run `stalwart-cli prepare` first.");
        return Ok(());
    }
    ...
    if stale > 0 {
        println!("Run `stalwart-cli prepare` to update the cache.");
        std::process::exit(1);
    }
    Ok(())
}
...
/// `query_scalar!("SQL" ...)`, and their `stalwart::` prefixed variants.
