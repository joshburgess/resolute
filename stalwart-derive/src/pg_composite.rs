...
pub fn derive(input: DeriveInput) -> TokenStream {
    ...
    let encode_fields: Vec<_> = fields
        ...
        .map(|f| {
            ...
            if let Some(inner_type) = extract_option_inner(field_type) {
                quote! {
                    buf.extend_from_slice(&(<#inner_type as stalwart::PgType>::OID).to_be_bytes());
                    match &self.#field_name {
                        Some(v) => stalwart::Encode::encode_param(v, buf),
                        None => buf.extend_from_slice(&(-1i32).to_be_bytes()),
                    }
                }
            } else {
                quote! {
                    buf.extend_from_slice(&(<#field_type as stalwart::PgType>::OID).to_be_bytes());
                    stalwart::Encode::encode_param(&self.#field_name, buf);
                }
            }
        })
        .collect();
            ...
            if let Some(inner_type) = extract_option_inner(field_type) {
                quote! {
                    ...
                    let #field_name = if __field_len == -1 {
                        ...
                        let __val = <#inner_type as stalwart::Decode>::decode(
                ...
                quote! {
                    #read_header
                    let #field_name = if __field_len == -1 {
                        return Err(stalwart::TypedError::UnexpectedNull(#idx));
                    } else {
                        let __l = __field_len as usize;
                        if __offset + __l > buf.len() {
                            return Err(stalwart::TypedError::Decode {
                        ...
                        let __val = <#field_type as stalwart::Decode>::decode(
                            &buf[__offset..__offset + __l],
                        )?;
                        __offset += __l;
                        __val
    ...
    let expanded = quote! {
        ...
        impl #impl_generics stalwart::DecodeText for #name #ty_generics #where_clause {
            fn decode_text(_s: &str) -> Result<Self, stalwart::TypedError> {
