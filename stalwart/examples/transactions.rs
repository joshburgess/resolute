...
//!
//! Run: cargo run -p stalwart --example transactions
//! Requires: docker compose up -d

use stalwart::{Client, Executor};

const ADDR: &str = "127.0.0.1:54322";

/// A generic function that works with any Executor (Client, Transaction, Pool).
async fn insert_item(db: &impl Executor, table: &str, id: i32, name: &str) -> Result<(), stalwart::TypedError> {
    db.execute(
