//! Tests for TypedPool (pg-pool + stalwart integration).
//! Requires: docker compose up -d (PostgreSQL on port 54322)

#![allow(dead_code)]

use stalwart::{Executor, TypedPool};
...
async fn test_typed_pool_from_row() {
    ...
    let authors: Vec<Author> = rows
        .iter()
        .map(|r| stalwart::FromRow::from_row(r).unwrap())
        .collect();
    ...
    let rows = client
        .query_named(
            ...
            &[
                ...
                ("name", &"pooled" as &dyn stalwart::SqlParam),
            ],
        )
        .await
        .unwrap();
    ...
    let count = client
        .execute_named(
            ...
            &[
                ...
                ("val", &"hello" as &dyn stalwart::SqlParam),
            ],
        )
        .await
        .unwrap();
