...
async fn test_query_macro_select_literal() {
    ...
    let rows = stalwart::query!("SELECT 42::int4 AS answer")
        .fetch_all(&client)
        .await
        .unwrap();
...
async fn test_query_macro_with_param() {
    let client = connect().await;
    let id = 1i32;
    let row = stalwart::query!("SELECT id, name FROM api.authors WHERE id = $1", id)
        .fetch_one(&client)
        .await
        .unwrap();
    assert_eq!(row.id, 1);
...
async fn test_query_macro_multiple_params() {
    ...
    let a = 3i32;
    let b = 4i32;
    let row = stalwart::query!("SELECT ($1::int4 + $2::int4) AS sum", a, b)
        .fetch_one(&client)
        .await
        .unwrap();
    assert_eq!(row.sum, 7);
...
async fn test_query_macro_fetch_all() {
    let client = connect().await;
    let rows = stalwart::query!("SELECT id, name FROM api.authors ORDER BY id")
        .fetch_all(&client)
        .await
        .unwrap();
    assert!(rows.len() >= 3);
...
async fn test_query_macro_fetch_opt_some() {
    let client = connect().await;
    let id = 1i32;
    let row = stalwart::query!("SELECT name FROM api.authors WHERE id = $1", id)
        .fetch_opt(&client)
        .await
        .unwrap();
    assert!(row.is_some());
...
async fn test_query_macro_text_type() {
    ...
    let rows = stalwart::query!("SELECT id FROM api.authors WHERE name = $1", name)
        .fetch_all(&client)
        .await
        .unwrap();
    assert_eq!(rows.len(), 1);
...
async fn test_query_macro_bigint() {
    ...
    let row = stalwart::query!("SELECT $1::int8 AS n", val)
        .fetch_one(&client)
        .await
        .unwrap();
...
async fn test_query_as_macro() {
    let client = connect().await;
    let id = 1i32;
    let author = stalwart::query_as!(MacroAuthor, "SELECT id, name FROM api.authors WHERE id = $1", id)
        .fetch_one(&client)
        .await
        .unwrap();
...
async fn test_query_as_macro_fetch_all() {
    let client = connect().await;
    let authors = stalwart::query_as!(MacroAuthor, "SELECT id, name FROM api.authors ORDER BY id")
        .fetch_all(&client)
        .await
        .unwrap();
...
async fn test_query_scalar_count() {
    let client = connect().await;
    let count = stalwart::query_scalar!("SELECT count(*)::int4 FROM api.authors")
        .fetch_one(&client)
        .await
        .unwrap();
    assert!(count >= 3);
...
async fn test_query_scalar_with_param() {
    let client = connect().await;
    let id = 1i32;
    let name = stalwart::query_scalar!("SELECT name FROM api.authors WHERE id = $1", id)
        .fetch_one(&client)
        .await
        .unwrap();
...
async fn test_query_scalar_bool() {
    let client = connect().await;
    let exists = stalwart::query_scalar!("SELECT exists(SELECT 1 FROM api.authors WHERE id = 1)")
        .fetch_one(&client)
        .await
        .unwrap();
    assert!(exists);
...
async fn test_query_file() {
    let client = connect().await;
    let id = 1i32;
    let row = stalwart::query_file!("tests/sql/get_author.sql", id)
        .fetch_one(&client)
        .await
        .unwrap();
    assert_eq!(row.id, 1);
...
async fn test_query_file_as() {
    let client = connect().await;
    let id = 1i32;
    let author = stalwart::query_file_as!(MacroAuthor, "tests/sql/get_author.sql", id)
        .fetch_one(&client)
        .await
        .unwrap();
    assert_eq!(author.id, 1);
...
async fn test_query_file_scalar() {
    let client = connect().await;
    let count = stalwart::query_file_scalar!("tests/sql/count_authors.sql")
        .fetch_one(&client)
        .await
        .unwrap();
    assert!(count >= 3);
...
async fn test_query_unchecked() {
    let client = connect().await;
    let rows = stalwart::query_unchecked!("SELECT 42::int4 AS n")
        .fetch_all(&client)
        .await
        .unwrap();
    assert_eq!(rows.len(), 1);
...
async fn test_query_unchecked_with_params() {
    let client = connect().await;
    let id = 1i32;
    let row = stalwart::query_unchecked!("SELECT name FROM api.authors WHERE id = $1", id)
        .fetch_one(&client)
        .await
        .unwrap();
...
async fn test_nullable_column_detection() {
    ...
    let row = stalwart::query!("SELECT id, name, bio FROM api.authors WHERE id = $1", 1i32)
        .fetch_one(&client)
        .await
        .unwrap();
    assert_eq!(row.id, 1);
...
async fn test_query_macro_named_params() {
    let client = connect().await;
    let id = 1i32;
    let row = stalwart::query!("SELECT id, name FROM api.authors WHERE id = :id", id = id)
        .fetch_one(&client)
        .await
        .unwrap();
    assert_eq!(row.id, 1);
...
async fn test_query_scalar_named() {
    let client = connect().await;
    let id_val = 1i32;
    let count = stalwart::query_scalar!(
