...
async fn test_timestamp_infinity() {
    ...
    let rows = client
        ...
        .await
        .unwrap();
    let ts: stalwart::PgTimestamp = rows[0].get(0).unwrap();
    assert_eq!(ts, stalwart::PgTimestamp::Infinity);
}
...
async fn test_timestamp_neg_infinity() {
    ...
    let rows = client
        ...
        .unwrap();
    let ts: stalwart::PgTimestamp = rows[0].get(0).unwrap();
    assert_eq!(ts, stalwart::PgTimestamp::NegInfinity);
}
...
async fn test_date_infinity() {
    ...
    let rows = client
        ...
        .await
        .unwrap();
    let d: stalwart::PgDate = rows[0].get(0).unwrap();
    assert_eq!(d, stalwart::PgDate::Infinity);
}
...
#[stalwart::test]
async fn test_stalwart_test_macro(client: stalwart::Client) {
    client
...
impl stalwart::FromRow for Author {
...
async fn test_numeric_roundtrip() {
    ...
    let rows = client
        ...
        .await
        .unwrap();
    let n: stalwart::PgNumeric = rows[0].get(0).unwrap();
...
async fn test_inet_roundtrip() {
    ...
    let rows = client
        ...
        .await
        .unwrap();
    let addr: stalwart::PgInet = rows[0].get(0).unwrap();
...
#[derive(Debug, PartialEq, stalwart::PgComposite)]
struct Point2D {
    x: f64,
    y: f64,
}
...
async fn test_database_create_and_connect() {
    let db_name = "stalwart_test_db_lifecycle";
...
fn test_decode_numeric_truncated() {
    ...
    assert!(stalwart::newtypes::PgNumeric::decode(&buf).is_err());
}

#[test]
fn test_decode_inet_truncated() {
    let buf = [2, 24, 0]; // family=IPv4 but too short
    assert!(stalwart::newtypes::PgInet::decode(&buf).is_err());
}
...
async fn test_query_named_basic() {
    ...
    let rows = client
        .query_named(
            "SELECT :id::int4 AS n",
            &[("id", &42i32 as &dyn stalwart::SqlParam)],
        )
        .await
        .unwrap();
    assert_eq!(rows[0].get::<i32>(0).unwrap(), 42);
...
async fn test_query_named_multiple() {
    ...
    let rows = client
        .query_named(
            ...
            &[
                ("a", &10i32 as &dyn stalwart::SqlParam),
                ("b", &"hello" as &dyn stalwart::SqlParam),
            ],
        )
        .await
        .unwrap();
...
async fn test_query_named_duplicate() {
    ...
    let rows = client
        .query_named(
            ...
            &[("val", &5i32 as &dyn stalwart::SqlParam)],
        )
        .await
        .unwrap();
...
async fn test_query_named_with_cast() {
    ...
    let rows = client
        .query_named(
            ...
            &[("value", &99i32 as &dyn stalwart::SqlParam)],
        )
        .await
        .unwrap();
...
async fn test_execute_named() {
    ...
    let count = client
        .execute_named(
            ...
            &[
                ("id", &1i32 as &dyn stalwart::SqlParam),
                ("name", &"Alice" as &dyn stalwart::SqlParam),
            ],
        )
        .await
        .unwrap();
...
async fn test_query_named_missing_param_errors() {
    ...
    let result = client
        .query_named(
            "SELECT :id::int4 AS n",
            &[("wrong_name", &42i32 as &dyn stalwart::SqlParam)],
        )
        .await;
    assert!(result.is_err());
}
...
async fn test_query_named_in_transaction() {
    ...
    txn.execute_named(
        ...
        &[("val", &7i32 as &dyn stalwart::SqlParam)],
    )
    .await
    .unwrap();
...
async fn insert_row(db: &impl Executor, table: &str, id: i32, val: &str) {
    db.execute_named(
        ...
        &[
            ("id", &id as &dyn stalwart::SqlParam),
            ("val", &val.to_string() as &dyn stalwart::SqlParam),
        ],
    )
    .await
    .unwrap();
...
async fn test_executor_trait_with_pool() {
    let pool = stalwart::TypedPool::connect(ADDR, USER, PASS, DB, 3)
        .await
        .unwrap();
    ...
    let result: Result<(), _> = client
        .with_transaction(|db| {
            Box::pin(async move {
                ...
                Err(stalwart::TypedError::Decode {
                    column: 0,
...
async fn atomic_insert(db: &impl Executor, table: &str, id: i32, val: &str) -> Result<(), stalwart::TypedError> {
    db.atomic(|db| {
        ...
        .atomic(|db| {
            Box::pin(async move {
                db.execute("INSERT INTO test_atomic_rb VALUES ($1)", &[&1i32])
                    .await?;
                // Force an error.
                Err::<(), _>(stalwart::TypedError::Decode {
                    column: 0,
...
async fn test_atomic_with_generic_executor_functions() {
    ...
    async fn batch_insert(db: &impl Executor) -> Result<(), stalwart::TypedError> {
        db.atomic(|db| {
            Box::pin(async move {
...
async fn test_pipeline_basic() {
    ...
    if let stalwart::PipelineResult::Rows(rows) = &results[0] {
        assert_eq!(rows[0].get::<i32>(0).unwrap(), 1);
    } else {
        panic!("expected Rows");
    }
    if let stalwart::PipelineResult::Rows(rows) = &results[1] {
        assert_eq!(rows[0].get::<String>(0).unwrap(), "hello");
    } else {
        panic!("expected Rows");
    }
...
async fn test_pipeline_mixed() {
    ...
    if let stalwart::PipelineResult::Execute(n) = results[0] {
        assert_eq!(n, 1);
    } else {
        panic!("expected Execute");
    }
    if let stalwart::PipelineResult::Rows(rows) = &results[2] {
        assert_eq!(rows[0].get::<i32>(0).unwrap(), 2);
    } else {
        panic!("expected Rows");
    }
...
async fn test_retry_policy_succeeds_immediately() {
    let client = connect().await;
    let policy = stalwart::retry::RetryPolicy::new(3, std::time::Duration::from_millis(10));
    let rows = policy
        .execute(&client, |db| {
...
async fn test_stress_pool_concurrent() {
    ...
    let pool = std::sync::Arc::new(
        stalwart::TypedPool::connect(ADDR, USER, PASS, DB, 5)
            .await
            .unwrap(),
    );
...
async fn test_stress_pool_mixed_operations() {
    ...
    for task_id in 0..20u32 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let rows = conn
                .query_named("SELECT :num::int4 AS n", &[("num", &(task_id as i32) as &dyn stalwart::SqlParam)])
                .await
                .unwrap();
...
fn test_pg_timestamp_encode_decode_roundtrip() {
    use stalwart::{PgTimestamp, Encode, Decode};
...
fn test_pg_date_encode_decode_roundtrip() {
    use stalwart::{PgDate, Encode, Decode};
...
async fn test_pool_warm_up() {
    let pool = stalwart::TypedPool::connect(ADDR, USER, PASS, DB, 5)
        .await
        .unwrap();
...
async fn test_pool_discard_all_clears_state() {
    let pool = stalwart::TypedPool::connect(ADDR, USER, PASS, DB, 1)
        .await
        .unwrap();
...
async fn test_metrics_recording() {
    ...
    let before = stalwart::metrics::snapshot().query_count;

    let client = connect().await;
    let _rows = client.query("SELECT 1::int4 AS n", &[]).await.unwrap();

    let after = stalwart::metrics::snapshot().query_count;
    assert!(
        after > before,
...
async fn test_test_db_lifecycle() {
    let db = stalwart::test_db::TestDb::create(ADDR, USER, PASS)
        .await
        .unwrap();
